  ...
  */
  ...
  getAddress=()=>{
    ...
    if(context.state.text.trim().length<4){
      context.props.navigation.dispatch(ToastActionsCreators.displayInfo("Please enter minimum 4 digit postal code."));
      return;
    }
  ...
  */
  onRequestPress = (data) => {
    ...
    let position = {
      lat: data.latitude,
      lng: data.longitude,
      address: (data.line_1.capitalizeEachLetter() +" "+ data.line_2.capitalizeEachLetter()),
      ...
      <TouchableOpacity 
        ...
        onPress={()=>this.onRequestPress(item)}>
          <Text style={{ color:Constants.Colors.Black }} numberOfLines={3}>{
            item.line_1.capitalizeEachLetter() +" "+item.line_2.capitalizeEachLetter() +" "+ 
  ...
  render() {
    ...
    return (
      <View style={[defaultStyles.container,]}>
        ...
        <FlatList
          data={context.props.location.addresses.length>0?context.props.location.addresses:[]}
