...
exports.googleMapNavigate = googleMapNavigate;
exports.decodeMapPoints = decodeMapPoints;
    ...
    fetch(directionsUrl)
      ...
      .then(function(responseJson) {
        if (responseJson.status === "OK") {
          ...
          var steps = [];
          if (
            (0, _Idx2.default)(responseJson, function(_) {
              return _.routes[0].legs[0].steps;
            })
          ) {
            steps = responseJson.routes[0].legs[0].steps;
          }
          if (responseJson.routes.length) {
            var _steps = decodeMapPoints(
              responseJson.routes[0].overview_polyline.points
            );
            return fulfill(_steps);
          }
        } else {
          reject();
        }
...
function googleMapNavigate(source, destination) {
  var url = "";
  url =
...
function calculateDistance(source, destination) {
  var unit = "N";
  var radlat1 = Math.PI * source.lat / 180;
  var radlat2 = Math.PI * destination.lat / 180;
  var theta = source.lng - destination.lng;
